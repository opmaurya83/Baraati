...
public class ContactsProvider {
    ...
    public ContactsProvider() {
        contentResolver = BaraatiApp.getContext().getContentResolver();
    }
    ...
    private void getPhone(Cursor cursor, String contactId, Contact contact) {
        ...
        if (hasPhoneNumber > 0) {
            ...
            while (phoneCursor.moveToNext()) {
                ...
                if(GH.check_prefix(phoneNumber,"00",2) || GH.check_prefix(phoneNumber,"+",1))
                {
                    if(GH.check_prefix(phoneNumber,"00",2))
                    {
                ...
                {
                    if(GH.check_prefix(phoneNumber,"0",1))
                    {
                        phoneNumber="91"+phoneNumber.substring(1,phoneNumber.length());
                    }
                    else
                    {
