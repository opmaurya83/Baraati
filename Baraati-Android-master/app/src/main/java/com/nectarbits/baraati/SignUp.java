...
public class SignUp extends BaseAuthActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    void OnSignUp() {
        if (GH.isInternetOn(this)) {
            if (!GH.isValidString(txtName.getText().toString().trim())) {
                L.showToast(mContext, getString(R.string.str_name_must));
            }
           /* else if (!GH.isValidName(txtName.getText().toString().trim())) {
                L.showToast(mContext, getString(R.string.str_name_range));
            }*/
            else if (txtEmailAddress.getText().toString().trim().equals("")) {
                L.showToast(mContext, getString(R.string.str_email_must));
            } else if (!GH.isValidEmail(txtEmailAddress.getText().toString().trim())) {
                L.showToast(mContext, getString(R.string.str_enter_valid_email));
            } else if (txtPassword.getText().toString().trim().equals("")) {
                L.showToast(mContext, getString(R.string.str_password_must));
            } else if (!GH.isValidPassword(txtPassword.getText().toString().trim())) {
                L.showToast(mContext, getString(R.string.str_password_should_atleast_6_character));
   ...
   /* @OnClick(R.id.btnBack)
    ...
    //optional
    @AskGranted(android.Manifest.permission.READ_CONTACTS)
...
/*        Loginusing:0 means normal login
        ...
        call.enqueue(new Callback<LoginModel>() {
            ...
            public void onResponse(Call<LoginModel> call, Response<LoginModel> response) {
                ...
                if (loginResponse.getStatus().equals("Success")) {
                    ...
                    Ask.on(SignUp.this)
                            .forPermissions(android.Manifest.permission.READ_CONTACTS)
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        } else if (requestCode == AppUtils.REQUEST_LOGIN_FROM_LS) {
...
*/
    ...
    private void performSignUpSuccessAction(Bundle bundle) {
        ...
        QBUser user = (QBUser) bundle.getSerializable(QBServiceConsts.EXTRA_USER);
        QBUpdateUserCommand.start(SignUp.this, user, image);
    ...
    private void onUserCheck() {
        ...
        if (!progressDialog.isShowing())
            progressDialog.show();
        JSONObject jsonObject = RequestBodyBuilder.getInstanse().Request_UserExistance(txtEmailAddress.getText().toString(), "91" + txtPhoneNumber.getText().toString());
        ...
        call.enqueue(new Callback<UserCheckModel>() {
            ...
            public void onResponse(Call<UserCheckModel> call, Response<UserCheckModel> response) {
                ...
                if (registerResponseInfo.getStatus().equals("Success")) {
                    ...
                    if (i == 0) {
                        RegisterSignletone.getInstance().setCode("");
                        RegisterSignletone.getInstance().setEmailid(txtEmailAddress.getText().toString());
                        RegisterSignletone.getInstance().setName(txtName.getText().toString());
                        RegisterSignletone.getInstance().setPassword(txtPassword.getText().toString());
                        RegisterSignletone.getInstance().setPhonenumber("91" + txtPhoneNumber.getText().toString());
                        startActivityForResult(new Intent(mContext, SignUpWithOTP.class), AppUtils.REQUEST_REGISTER);
    ...
    @OnClick({R.id.txtterms, R.id.txtprivacy,R.id.tvtLoginwithfacebook,R.id.tvtLoginwithgoogle})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.txtprivacy:
                ...
                break;
            case R.id.tvtLoginwithfacebook:
                if (GH.isInternetOn(mContext))
                    loginButton.performClick();
                break;
            case R.id.tvtLoginwithgoogle:
                if (GH.isInternetOn(mContext))
                    signIn();
                break;
        }
